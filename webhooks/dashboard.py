...
@router.post("/query")
async def api_db_query(payload: dict = Body(...)):
    """Execute a read-only SELECT query against ivr_call_records. Returns up to 200 rows."""
    sql = payload.get("sql", "").strip()
    if not sql.upper().lstrip().startswith("SELECT"):
        return JSONResponse({"ok": False, "error": "Only SELECT queries are permitted."}, status_code=400)
    import services.call_db as _db
    conn = _db._get_conn()
    if conn is None:
        return JSONResponse({"ok": False, "error": "Database not available (Postgres unreachable)."}, status_code=503)
    try:
        with conn.cursor() as cur:
            ...
            return JSONResponse({
                "ok":      True,
                "columns": cols,
                "rows":    [[_serialize(c) for c in r] for r in rows],
                "count":   len(rows),
            })
...
/* ── DB QUERY ── */
.db-wrap{display:flex;width:100%;height:100%}
.db-side{width:270px;background:#1e293b;border-right:1px solid #334155;display:flex;flex-direction:column;flex-shrink:0;overflow-y:auto}
.db-side-hdr{padding:10px 14px;font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.06em;border-bottom:1px solid #334155;flex-shrink:0}
.db-sample{padding:9px 14px;cursor:pointer;border-bottom:1px solid rgba(255,255,255,.04);font-size:11px;color:#94a3b8;line-height:1.5}
.db-sample:hover{background:rgba(255,255,255,.04);color:#e2e8f0}
.db-sample-name{font-weight:700;color:#38bdf8;display:block;font-size:11px;margin-bottom:2px}
.db-main{flex:1;display:flex;flex-direction:column;overflow:hidden}
.db-editor{flex-shrink:0;border-bottom:1px solid #334155}
.db-sql{width:100%;background:#0a0f1a;color:#e2e8f0;border:none;padding:14px 16px;font-family:Consolas,'Courier New',monospace;font-size:12px;line-height:1.6;resize:vertical;outline:none;min-height:110px;max-height:240px;display:block}
.db-toolbar{display:flex;align-items:center;gap:8px;padding:7px 14px;border-bottom:1px solid #334155;flex-shrink:0;background:#111827}
.db-run-btn{background:#3b82f6;color:#fff;border:none;border-radius:6px;padding:5px 16px;font-size:12px;font-weight:700;cursor:pointer}
.db-run-btn:hover{background:#2563eb}
.db-run-btn:disabled{opacity:.4;cursor:not-allowed}
.db-clear-btn{background:#334155;color:#94a3b8;border:none;border-radius:6px;padding:5px 12px;font-size:12px;cursor:pointer}
.db-clear-btn:hover{background:#475569;color:#e2e8f0}
.db-status{font-size:11px;color:#64748b;margin-left:6px;flex:1}
.db-results{flex:1;overflow:auto}
.db-tbl{width:100%;border-collapse:collapse;font-size:12px;font-family:Consolas,'Courier New',monospace}
.db-tbl th{background:#1e293b;color:#38bdf8;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;padding:7px 12px;border-bottom:1px solid #334155;text-align:left;position:sticky;top:0;white-space:nowrap}
.db-tbl td{padding:5px 12px;border-bottom:1px solid rgba(255,255,255,.04);color:#e2e8f0;white-space:pre;max-width:340px;overflow:hidden;text-overflow:ellipsis;vertical-align:top}
.db-tbl tr:hover td{background:rgba(255,255,255,.03)}
.db-empty{padding:48px;color:#475569;text-align:center;font-size:13px}
.db-error{padding:16px 20px;color:#f87171;font-size:12px;font-family:monospace;white-space:pre-wrap;line-height:1.6}
</style>
</head>
<body>
...
<div class="tabbar">
  ...
  <div class="tab" data-pane="db">🗄 DB Query</div>
</div>
...
<div class="pane" id="pane-db">
  <div class="db-wrap">
    <div class="db-side">
      <div class="db-side-hdr">Sample Queries</div>
      <div id="db-sample-list"></div>
    </div>
    <div class="db-main">
      <div class="db-editor">
        <textarea class="db-sql" id="db-sql" placeholder="SELECT call_sid, started_at, data->>'status' AS status FROM ivr_call_records ORDER BY started_at DESC LIMIT 20;"></textarea>
      </div>
      <div class="db-toolbar">
        <button class="db-run-btn" id="db-run-btn" onclick="dbRun()">▶ Run</button>
        <button class="db-clear-btn" onclick="dbClear()">Clear</button>
        <span class="db-status" id="db-status">Enter a SELECT query and click Run. Results capped at 200 rows.</span>
      </div>
      <div class="db-results" id="db-results">
        <div class="db-empty">No results yet.</div>
      </div>
    </div>
  </div>
</div>
